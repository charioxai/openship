...
r.post(
  "/folder/session",
  {
    tag: "project:write",
    collection: true,
    // A create-scoped PAT needs an isolated staging session before the project
    // exists. The session is bound to the authenticated user + org; it grants
    // no access to any existing project.
    projectCreate: true,
...
r.post(
  "/folder/scan/:sessionId",
  {
    tag: "project:write",
    collection: true,
    // Same create-scoped staging boundary as folder/session. The controller
    // also requires the session's org to match the caller.
    projectCreate: true,
    mcp: {
      description:
