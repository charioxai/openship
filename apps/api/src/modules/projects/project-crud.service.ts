...
export function staleServiceKindForProjectShape(
  data: Pick<EnsureProjectBody, "projectType" | "services" | "monorepoApps">,
): "compose" | "monorepo" | null {
  if (data.projectType === "services" && data.services?.length) return "monorepo";
  if (data.projectType === "monorepo" && data.monorepoApps?.length) return "compose";
  return null;
}
